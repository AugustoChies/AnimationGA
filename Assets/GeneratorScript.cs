...
public class GeneratorScript : MonoBehaviour {
    ...
    public Vector3 coneend; //circle that marks the area the particle is "aimed" at
    ...
    public void Burst(int ammount)
    {
        for(int i=0; i < ammount;i++)
        {
            ...
            if (trajectory == TrajectoryType.cone)
            {
                direction.x = Random.Range(-coneend.x, coneend.x);
                direction.y = coneend.y;
                direction.z = Random.Range(-coneend.z, coneend.z);
            }
